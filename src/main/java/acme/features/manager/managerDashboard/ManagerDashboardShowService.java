...
public class ManagerDashboardShowService extends AbstractService<Manager, ManagerDashboard> {
	...
	public void load() {
		...
		Collection<UserStory> userStories = this.repository.findManyUserStoriesByManagerId(managerId).stream().filter(x -> x.isPublished()).toList();
		...
		double totalUserStoriesWithPriorityMust = userStories.stream().filter(x -> x.getPriority() == Priority.MUST).count();
		double totalUserStoriesWithPriorityShould = userStories.stream().filter(x -> x.getPriority() == Priority.SHOULD).count();
		double totalUserStoriesWithPriorityCould = userStories.stream().filter(x -> x.getPriority() == Priority.COULD).count();
		double totalUserStoriesWithPriorityWont = userStories.stream().filter(x -> x.getPriority() == Priority.WONT).count();
